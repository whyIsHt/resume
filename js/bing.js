...
function pieChart() {
  ...
  var sliceBorderStyle = "#333333";                    // Colour of the border around each slice
  ...
  function startPullOut ( slice ) {
    ...
    $('#chartData tr').children("td").each(function(index){
      if(index % 2 ==0){
        var flag = ".b_"+$(this).html().toLowerCase();
        $(flag).asPieProgress('reset');
      }
    });
    var str = ".b_"+$('#chartData td:eq(' + (slice*2) + ')').html().toLowerCase();
    console.log(str);
    $(str).asPieProgress('go',100);
  }
